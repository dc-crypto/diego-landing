    ...
    <div className={`${fraunces.variable} ${workSans.variable} ${spaceMono.variable}`}
      ...
      <section id="ubicaciones" style={{ padding: "72px 0" }}>
        <div style={{ maxWidth: "1080px", margin: "0 auto", padding: "0 32px" }}>
            ...
            {[
              {
                ...
                address: "C. Palma Sola 1625, 18 de Marzo, 44960 Guadalajara, Jal.",
                phone: "33 3709 9566",
                hours: "Desde 1:00 p.m.",
                note: "Terraza al aire libre, cócteles, música en vivo",
                wa: WA1,
                mapQ: "C.+Palma+Sola+1625,+18+de+Marzo,+44960+Guadalajara,+Jal.",
              },
              {
                num: "02", name: "Tesoro",
                address: "Vista a la Campiña 617, Cerro del Tesoro, Tlaquepaque, Jal.",
                phone: "33 3113 3453",
                hours: "Por confirmar",
                note: "Ambiente familiar, fácil acceso desde Tlaquepaque",
                wa: WA2,
              ...
              <Reveal key={loc.name} delay={(i + 1) as 1 | 2}>
                {/* Ticket card */}
                <div style={{ backgroundColor: C.parchment, color: C.char, borderRadius: "2px", overflow: "hidden" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr auto" }}>
                    <div style={{ padding: "26px 24px" }}>
                      <div style={{ fontFamily: mono, fontSize: "11px", letterSpacing: "0.1em", textTransform: "uppercase", color: C.ember, marginBottom: "4px" }}>
                        Sucursal {loc.num} · Pizzería
                      </div>
                      <h3 style={{ fontFamily: serif, fontWeight: 500, fontSize: "24px", color: C.char, margin: "0 0 16px" }}>{loc.name}</h3>
                      {[
                        { label: "Dirección", val: loc.address },
                        { label: "Teléfono",  val: loc.phone },
                        { label: "Horario",   val: loc.hours },
                        { label: "Destaca",   val: loc.note },
                      ].map((row) => (
                        <div key={row.label} style={{ display: "flex", gap: "8px", fontSize: "13px", marginBottom: "6px", color: "#4a3f36" }}>
                          <b style={{ color: C.char, fontWeight: 600, minWidth: "70px", fontFamily: sans }}>{row.label}</b>
                          <span style={{ fontFamily: sans }}>{row.val}</span>
                        </div>
                      ))}
                      <a href={loc.wa} target="_blank" rel="noopener noreferrer"
                        className="btn-press"
                        style={{ display: "inline-flex", alignItems: "center", gap: "6px", marginTop: "14px", fontFamily: sans, fontSize: "13px", fontWeight: 500, padding: "10px 18px", border: `1px solid ${C.char}`, borderRadius: "2px", color: C.char, transition: "background-color 0.15s, color 0.15s" }}
                        onMouseEnter={(e) => { const el = e.currentTarget as HTMLAnchorElement; el.style.backgroundColor = C.char; el.style.color = C.parchment; }}
                        onMouseLeave={(e) => { const el = e.currentTarget as HTMLAnchorElement; el.style.backgroundColor = "transparent"; el.style.color = C.char; }}>
                        Escribir por WhatsApp
                      </a>
                    </div>
                    {/* Dashed stub */}
                    <div style={{
                      backgroundColor: C.char, color: C.parchment,
                  ...
                  {/* Google Map */}
                  <div style={{ borderTop: `2px dashed rgba(28,19,16,0.15)` }}>
                    <iframe
          ...
          <p style={{ fontFamily: sans, fontSize: "13px", color: C.parchmentDim, marginTop: "24px" }}>
            Horario completo (lun–dom) pendiente de confirmar en la ficha de Google.
          </p>
        </div>
      </section>
